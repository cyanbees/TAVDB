...
async function loadList(params) {
  ...
  if (params.genreId) return search({ keyword: params.genreId, baseUrl: params.baseUrl });
  if (params.peopleId) return search({ keyword: params.peopleId, baseUrl: params.baseUrl });
...
async function search(params) {
  ...
  var html = await requestHtml(baseUrl + path);
  return await parseList(html, baseUrl, coverProxy, 0, "image");
}
...
async function loadDetail(link) {
  ...
  return {
    ...
    link: encodeLink({ url: url, baseUrl: baseUrl, coverProxy: coverProxy, title: cleanText(title) }),
...
async function hydrateCovers(items, baseUrl, coverProxy) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    item.coverUrl = coverProxy + "?url=" + encodeURIComponent(item.coverUrl);

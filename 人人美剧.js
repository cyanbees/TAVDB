...
const VIP_TOKEN = 'rrtv-05188573836c6a02cce84f931ad161282cf039b4';

async function loadResource(params) {
  const { seriesName, season, episode, type, multiSource } = params;
  ...
  try {
    ...
    const sRes = await Widget.http.get(`${searchUrl}?${sortedQueryString(sParams)}`, { headers: sHeaders, timeout: 8000 });
    ...
    const pRes = await Widget.http.get(`${pageUrl}?${sortedQueryString(pParams)}`, { headers: pHeaders, timeout: 8000 });
    ...
    // --- 4. 播放解析 -- 遍历所有可用清晰度 ---
    const qualityItems = pData.data.watchInfo.sortedItems || [];
    if (!qualityItems.length) return [];

    const playResults = [];
    for (const qItem of qualityItems) {
      const qCode = qItem.qualityCode || 'HD';
      try {
        ...
        // --- 获取并验证播放链接（含重定向跟随+失败重试） ---
        const plHeaders = buildSignedHeaders("GET", playUrl, plParams, deviceId, VIP_TOKEN);
        const plRes = await Widget.http.get(`${playUrl}?${sortedQueryString(plParams)}`, { headers: plHeaders, timeout: 8000 });
        const plData = JSON.parse(decrypt(plRes.data));
        
        let finalUrl = decryptPlayUrl(plData.data.m3u8.url, plData.data.newSign);
        const qLabel = plData.data.m3u8.currentQuality;
        const displayName = qLabel && qLabel !== qCode ? qCode + ' (' + qLabel + ')' : qCode;

        // 跟随重定向 + 失败自动重试（最多3次，轮换CDN节点）
        finalUrl = await resolvePlayUrlWithRetry(drama.id, episodeInfo.sid, qCode, deviceId, finalUrl);

        if (!finalUrl) {
          console.error('[人人美剧] ' + qCode + ' 所有CDN节点均不可用');
          continue;
        }

        playResults.push({
          name: displayName,
          description: seriesName + (type === 'tv' ? ' 第' + episode + '集' : '') + ' [' + displayName + ']',
          url: finalUrl
        });
      } catch (e) {
        console.error('[人人美剧] 清晰度 ' + qCode + ' 解析失败:', e.message || e);
      }
    }

    if (!playResults.length) return [];
    return playResults;

  } catch (err) {
    console.error('[人人美剧] loadResource error:', err.message || err);
    return [];
  }
}
...
/**
 * 跟随重定向并验证播放链接可用性，失败时自动刷新换CDN节点
 */
async function resolvePlayUrlWithRetry(dramaId, episodeSid, qualityCode, deviceId, initialUrl) {
  const MAX_ATTEMPTS = 3;
  
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    try {
      // 用 GET 请求验证CDN节点，Widget.http 自动跟随302跳转
      const testUrl = attempt === 0 ? initialUrl : await refreshPlayUrl(dramaId, episodeSid, qualityCode, deviceId);
      if (!testUrl) continue;
      
      // 用 Range 请求仅获取响应头+第1字节，避免下载完整MP4
      const resp = await Widget.http.get(testUrl, { 
        timeout: 5000,
        headers: {
      ...
      // Widget.http 自动跟随302跳转，resp.url 为最终CDN地址
      // 检查状态码：200(完整文件) 或 206(Partial Content) 都表示CDN节点可用
      if (resp && (resp.status === 200 || resp.status === 206)) {
        const finalUrl = resp.url || testUrl;
        console.log('[人人美剧] CDN节点可用: ' + finalUrl.substring(0, 80) + '...');
        return finalUrl;
      }
      
      console.log('[人人美剧] 尝试' + (attempt + 1) + ' CDN返回 ' + (resp ? resp.status : '无响应') + '，刷新重试');
    } catch (e) {
      console.log('[人人美剧] 尝试' + (attempt + 1) + ' CDN不可用: ' + (e.message || e) + '，刷新重试');
    }
  }
 ...
 * 重新请求播放API以获取新的CDN节点
 */
async function refreshPlayUrl(dramaId, episodeSid, qualityCode, deviceId) {
  try {
    const playUrl = `${SITE_API}/m-station/drama/play`;
    const plParams = { 
    ...
    if (!plData.data || !plData.data.m3u8 || !plData.data.m3u8.url) return null;
    
    return decryptPlayUrl(plData.data.m3u8.url, plData.data.newSign);
  } catch (e) {
    console.error('[人人美剧] 刷新播放链接失败:', e.message || e);
    return null;
  }
}

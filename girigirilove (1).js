...
function sortParam(sort) {
  ...
  if (sort === "score") return "sort=score";
  return "";
}
...
async function search(params) {
  try {
    ...
    // Search via POST form (page encoded in URL path)
    var searchUrl = SITE + "/search/-------------" + (page > 1 ? page : "") + "---/";
    var res = await Widget.http.post(searchUrl, "wd=" + encodeURIComponent(keyword), {
      headers: {
...
async function scanCategoryForTitle(wantBaseNorm, isMovie) {
  // Scan first page of each relevant category for best matching title
  var maxPages = 1; // 大部分番剧在第一页就能找到
  ...
  for (var ci = 0; ci < catIds.length && bestScore < 300; ci++) {
    ...
    for (var pg = 1; pg <= maxPages; pg++) {
      var url = SITE + "/show/" + catIds[ci] + "--------" + pg + "---/";
      pageReqs.push(
    ...
    for (var pi = 0; pi < pages.length && bestScore < 300; pi++) {
      ...
      for (var ii = 0; ii < items.length; ii++) {
        var sc = scoreResult(items[ii].title, wantBaseNorm, 0);
        if (sc > bestScore) {
          bestScore = sc;
          best = items[ii];
        }
...
async function loadResource(params) {
  try {
    ...
    var animeList = await scanCategoryForTitle(wantBaseNorm, isMovie);
    ...
    $(".anthology-tab a.swiper-slide").each(function (i) {
      ...
      if (text.indexOf("简中") !== -1) scIndex = i;
    });
...
async function loadDetail(link) {
  try {
    ...
    // Parse episode groups — use simplified Chinese (简中) if available
    var scIndex = -1;
    $(".anthology-tab a.swiper-slide").each(function (i) {
      var text = $(this).text().trim();
      if (text.indexOf("简中") !== -1) scIndex = i;
    ...
    // Fall back to first group if no 简中 found
    if (scIndex === -1) scIndex = 0;

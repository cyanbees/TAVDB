...
async function loadResource(params) {
  ...
  // ===== 搜索匹配（从原 loadResource 恢复）=====
  var seriesName = params.seriesName || params.title || "";
  ...
  logInfo("loadResource 搜索: " + searchKeyword + ", 类型: " + type + ", 集数: " + episode);

  return searchVodOle(apiHost, searchKeyword, 1).then(function(searchResults) {
    if (!searchResults.length) return [];

    var matchedItem = null, bestScore = 0;
    var cleanSearch = normalizeTitle(searchKeyword);

    for (var i = 0; i < searchResults.length; i++) {
      var item = searchResults[i];
      if (item.vip && !GLOBAL_COOKIE) continue;
      var nameNorm = normalizeTitle(item.vod_name);
      var score = 0;
      if (nameNorm === cleanSearch) score = 100;
      else if (nameNorm.indexOf(cleanSearch) >= 0 || cleanSearch.indexOf(nameNorm) >= 0) score = 60;
      if (score > bestScore) { bestScore = score; matchedItem = item; }
    }

    if (!matchedItem || bestScore < 50) return [];

    return getDetailOle(apiHost, matchedItem.vod_id).then(function(detail) {
      if (!detail || !detail.urls || !detail.urls.length) return [];

      var targetEpNo = type === "movie" ? null : episode;
      var matchedEpisodes = [];
      for (var i = 0; i < detail.urls.length; i++) {
        var item = detail.urls[i];
        if (!GLOBAL_COOKIE && item.vip === true) continue;
        var epName = item.title || "";
        var epNo = extractEpisodeNumber(epName);
        if (targetEpNo !== null && epNo !== null && epNo !== targetEpNo) continue;
        var videoUrl = item.url || item.play_url || item.link || "";
        if (!videoUrl) continue;
        matchedEpisodes.push({ title: epName, url: videoUrl, language: extractLanguage(item.remark || "", epName) });
      }
      if (!matchedEpisodes.length) return [];

      var resources = [];
      for (var i = 0; i < matchedEpisodes.length; i++) {
        var ep = matchedEpisodes[i];
        var description = matchedItem.vod_name;
        if (targetEpNo !== null) description += " 第" + targetEpNo + "集";
        else if (ep.title) description += " " + ep.title;
        if (ep.language) description += " [" + ep.language + "]";
        resources.push({
          name: "欧乐影视" + (targetEpNo !== null ? " 第" + targetEpNo + "集" : ""),
  ...
  if (detailId.includes("ole://detail")) {
    ...
    return getDetailOle(apiHost, vodId).then(function(detail) {
      ...
      for (var i = 0; i < detail.urls.length; i++) {
        ...
        var videoUrl = item.url || item.play_url || item.link || "";
        if (!videoUrl) continue;
        var epDisplayTitle = "欧乐影视 " + (rawTitle || "播放");
        episodeItems.push({
          id: vodId + "_" + i, type: "url", title: epDisplayTitle,
          videoUrl: videoUrl, mediaType: isMovie ? "movie" : "tv"
        });
      }
      if (episodeItems.length === 0) throw new Error("未找到可播放的链接");
      var mediaType = isMovie ? "movie" : "tv";
      var videoUrl = isMovie && episodeItems.length > 0 ? episodeItems[0].videoUrl : null;
      return {
